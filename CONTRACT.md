...
`appName`. **Known gap:** the worker currently reads its own local file,
`backend-api/data/rbi_apps.json` (placeholder/sample entries), which is a
*separate file* from the Node backend's `/data/rbi-regulated-lending-apps.json`
(also currently empty — see §4). They are not yet the same list, so the
worker's `lendingAppCheck` and the Node backend's standalone
`GET /api/lending-check` endpoint can disagree. Before the demo, either
point both at one real, populated RBI list, or drop one of the two lookup
paths — don't leave both live with different data.
...
`/data/rbi-regulated-lending-apps.json`:

```json
{ "source": "...", "lastUpdated": "YYYY-MM-DD" | null, "apps": ["App Name", "..."] }
```

Currently ships with an empty `apps` list — see
`backend/scripts/scrape-rbi-list.ts` for the TODO to populate it from
RBI's public DLA directory (rbi.org.in → Citizen's Corner → "DLAs
Deployed by Regulated Entities", live since 2025-07-01).
